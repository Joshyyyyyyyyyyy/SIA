...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'approve') {
        $stmt = $pdo->prepare("UPDATE reservations SET status = 'approved', approved_by = ?, approved_at = NOW() WHERE id = ?");
        $stmt->execute([$_SESSION['staff_id'], $reservation_id]);
        $_SESSION['message'] = 'Reservation approved successfully!';
        $_SESSION['message_type'] = 'success';
    } elseif ($_POST['action'] === 'cancel') {
        $reason = isset($_POST['cancel_reason']) ? sanitize($_POST['cancel_reason']) : '';
        $stmt = $pdo->prepare("UPDATE reservations SET status = 'cancelled', cancel_reason = ?, cancelled_by = ?, cancelled_at = NOW() WHERE id = ?");
        $stmt->execute([$reason, $_SESSION['staff_id'], $reservation_id]);
...
<body>
    ...
    <div class="modal" id="approveModal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">Approve Reservation</h3>
                    <button type="button" class="close-modal" onclick="closeModal('approveModal')">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <p>Are you sure you want to approve this reservation?</p>
                    <form action="reservation.php" method="POST" id="approveForm">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="close-modal" onclick="closeModal('cancelModal')">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="reservation.php" method="POST" id="cancelForm">
                        <input type="hidden" name="reservation_id" id="cancel_reservation_id">
                        <input type="hidden" name="action" value="cancel">
                        <div class="form-group">
                            <label for="cancel_reason">Reason for Cancellation</label>
                            <textarea name="cancel_reason" id="cancel_reason" rows="3" class="form-control" required></textarea>
                        </div>
                        <div class="form-actions">
    ...
    <script>
        ...
        function approveReservation(id) {
            document.getElementById('approve_reservation_id').value = id;
